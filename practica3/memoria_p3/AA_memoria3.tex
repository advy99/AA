...
Si tenemos $N$ datos en training, sabemos que:
...
Para esto usaremos \texttt{cross\_val\_score}\cite{crossval} usando tantos k-folds como datos tiene el conjunto de training para hacer el método Leave One Out, y realizar la estimación, con lo que obtenemos lo siguiente:

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{clasificacion/eout_clas.png}
	\caption{Estimación de $E_{out}$ usando cross-validation.}
	\label{eoutclas}
\end{figure}


Vemos como en este caso esta estimación está dentro de la cota calculada usando cross-validation ya que el error en el conjunto de test es menor que esta cota:
...
Como vemos, aunque es cierto que se obtienen mejores resultados con una regularización más fuerte, apenas hay cambios y el modelo obtiene valores muy buenos, manteniendo un error del 7\% fuera de la muestra.
...
Para la función de perdida usaremos square\_loss como hemos comentado.
...
\begin{figure}[H]
	\centering
	\hspace*{-1cm}\includegraphics[scale=0.4]{regresion/sgd001a0.png}
	\caption{Resultado usando el algoritmo SGD Regressor con tasa de aprendizaje constante 0.01 , sin regularización y función de perdida square\_loss.}
	\label{SGDL0}
\end{figure}

\begin{figure}[H]
	\centering
	\hspace*{-1cm}\includegraphics[scale=0.4]{regresion/sgd00001.png}
	\caption{Resultado usando el algoritmo SGD Regressor con tasa de aprendizaje optima (variable) y factor de regularización 0.0001 y función de perdida square\_loss.}
	\label{SGDL00001}
\end{figure}

\begin{figure}[H]
	\centering
	\hspace*{-1cm}\includegraphics[scale=0.4]{regresion/sgd00001c.png}
	\caption{Resultado usando el algoritmo SGD Regressor con tasa de aprendizaje constante y factor de regularización 0.0001 y función de perdida square\_loss.}
	\label{SGDL00001}
\end{figure}

\begin{figure}[H]
	\centering
	\hspace*{-1cm}\includegraphics[scale=0.4]{regresion/sgd001.png}
	\caption{Resultado usando el algoritmo SGD Regressor con tasa de aprendizaje optima (variable) y factor de regularización 0.01 y función de perdida square\_loss.}
	\label{SGDL001}
\end{figure}

\begin{figure}[H]
	\centering
	\hspace*{-1cm}\includegraphics[scale=0.4]{regresion/sgd001c.png}
	\caption{Resultado usando el algoritmo SGD Regressor con tasa de aprendizaje constante y factor de regularización 0.01 y función de perdida square\_loss.}
	\label{SGDL001}
\end{figure}
...
\begin{figure}[H]
	\centering
	\hspace*{-1cm}\includegraphics[scale=0.4]{regresion/sgd01.png}
	\caption{Resultado usando el algoritmo SGD Regressor con tasa de aprendizaje optima (variable) y factor de regularización 0.1 y función de perdida square\_loss.}
	\label{SGDL001}
\end{figure}

\begin{figure}[H]
	\centering
	\hspace*{-1cm}\includegraphics[scale=0.4]{regresion/sgd01c.png}
	\caption{Resultado usando el algoritmo SGD Regressor con tasa de aprendizaje constante y factor de regularización 0.1 y función de perdida square\_loss.}
	\label{SGDL001}
\end{figure}
...
\begin{figure}[H]
	\centering
	\hspace*{-1cm}\includegraphics[scale=0.4]{regresion/sgd1.png}
	\caption{Resultado usando el algoritmo SGD Regressor con tasa de aprendizaje optima (variable) y factor de regularización 1 y función de perdida square\_loss.}
	\label{SGDL001}
\end{figure}

\begin{figure}[H]
	\centering
	\hspace*{-1cm}\includegraphics[scale=0.4]{regresion/sgd1c.png}
	\caption{Resultado usando el algoritmo SGD Regressor con tasa de aprendizaje constante y factor de regularización 1 y función de perdida square\_loss.}
	\label{SGDL001}
\end{figure}


Para este problema vemos como el factor de regularización de 1 es demasiado alto para este algoritmo y obtenemos peores valores del error tanto en training como en test debido al infraajuste.


\subsubsection{Mejor modelo.}

Escogeré el mejor modelo según el modelo con mejor error en el conjunto de test ya que es el que mejor ha estimado unos datos con los que no ha entrenado. Por este motivo el modelo escogido será Regresión Ridge con un factor de regularización 2, por lo que la estimación del error se hará con este modelo.

\subsection{Estimación del error fuera de la muestra usando validación cruzada y comparación con el error en test.}

Como hemos visto tanto en teoría\cite{teoria} como en la bibliografía de la asignatura\cite{libro}, podemos hacer una estimación del error fuera de la muestra al usar validación cruzada.

Si tenemos $N$ datos, sabemos que:
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{regresion/eout_reg.png}
	\caption{Estimación de $E_{out}$ usando cross-validation.}
	\label{eoutreg}
\end{figure}

Vemos como esta estimación, comparándola con $E_{test}$ no se ajusta a la realidad, seguramente por la gran dimensionalidad del problema, este ajuste es erróneo y no se acerca a la realidad.


\subsection{Modelo propuesto y estimación del error fuera de la muestra de este modelo.}

El modelo prepuesto será el modelo con mejor $E_{test}$, es decir, Regresión Ridge con un factor de regularización de 2.

Podemos hacer una estimación del error fuera de la muestra usando el conjunto de test. En el modelo que hemos escogido vemos como esta estimación se de un error del 7\%.

Aun así, hemos obtenido muy buenos resultados para ser un problema con gran número de variables, pero si estas variables son necesarias y realmente aportan información han conseguido un mejor ajuste.
